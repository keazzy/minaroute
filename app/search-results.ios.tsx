...
import { NativeSheet, type NativeSheetRef } from '@/components/NativeSheet';
import PlaceCard from '@/components/place-card';
import { Colors } from '@/constants/theme';
import { useColorScheme } from '@/hooks/use-color-scheme';
import { BottomSheetBackdrop, BottomSheetModal, BottomSheetScrollView as GorhomBottomSheetScrollView } from '@gorhom/bottom-sheet';
...
import { Alert, Animated, Easing, Linking, Modal, PanResponder, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, useWindowDimensions } from 'react-native';
...
function ResultSkeletonCard({ isDark = false }: { isDark?: boolean }) {
  return (
    <View style={[styles.card, { backgroundColor: isDark ? '#2a2a2a' : '#f9f6f4' }]}>
      <ShimmerBlock style={styles.skeletonImageTile} isDark={isDark} />
...
export default function SearchResultsScreen() {
  ...
  // Sheet state - tracks if sheet is collapsed (showing map) or expanded (full list)
  const [isSheetExpanded, setIsSheetExpanded] = useState(true); // Start expanded (full list view)
  ...
  // Header height calculation (will be measured)
  ...
  // Native sheet refs for type and tags
  const typeSheetRef = useRef<NativeSheetRef>(null);
  const tagsSheetRef = useRef<NativeSheetRef>(null);

  const openTypePicker = useCallback(() => {
    typeSheetRef.current?.present();
  }, []);

  const openTagsPicker = useCallback(() => {
  ...
  // Sheet positioning
  const collapsedSheetTop = useMemo(() => {
    return windowHeight * 0.6; // Collapsed: sheet starts at 60% from top (shows 40% of screen)
  }, [windowHeight]);
  
  const expandedSheetTop = useMemo(() => {
    return headerHeight; // Expanded: sheet starts right below header
  }, [headerHeight]);
  
  // Animated sheet top position
  const sheetTop = sheetAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [collapsedSheetTop, expandedSheetTop],
  });
  ...
  // Filter places that have coordinates for map view
  const mappablePlaces = useMemo(() => {
    return filteredPlaces.filter((p): p is Place => {
      return (p as Place).coordinate != null;
    });
  }, [filteredPlaces]);

  // Calculate map region based on filtered places
  const mapRegion = useMemo(() => {
    if (mappablePlaces.length === 0) {
      return userCoords 
        ? { ...userCoords, latitudeDelta: 0.1, longitudeDelta: 0.1 }
        : INITIAL_REGION;
    }

    const lats = mappablePlaces.map((p) => p.coordinate.latitude);
    const lngs = mappablePlaces.map((p) => p.coordinate.longitude);
    ...
    const latDelta = Math.max(0.05, (maxLat - minLat) * 1.3);
    const lngDelta = Math.max(0.05, (maxLng - minLng) * 1.3);
  ...
  }, [mappablePlaces, userCoords]);

  const scrollViewRef = useRef<ScrollView>(null);
  const scrollOffsetY = useRef(0);
  const isSheetExpandedRef = useRef(true);
  
  // Keep ref in sync with state for use in PanResponder
  useEffect(() => {
  ...
  const expandSheet = useCallback(() => {
    ...
    Animated.spring(sheetAnim, {
      ...
      if (finished) setIsSheetExpanded(true);
    });
  }, [sheetAnim]);
  ...
  const collapseSheet = useCallback(() => {
    ...
    Animated.spring(sheetAnim, {
      ...
      if (finished) setIsSheetExpanded(false);
    });
  }, [sheetAnim]);
  ...
  // Track if we should capture the pan (for sheet drag vs scroll)
  const shouldCaptureRef = useRef(false);
  
  // PanResponder for the entire sheet area
  const panResponder = useMemo(() => PanResponder.create({
    onStartShouldSetPanResponder: () => {
      // Always capture when collapsed
      if (!isSheetExpandedRef.current) return true;
      // When expanded, only capture if at top of scroll
      return scrollOffsetY.current <= 0;
    },
    onMoveShouldSetPanResponder: (_, gestureState) => {
      // When collapsed, capture any vertical drag
      if (!isSheetExpandedRef.current) {
        return Math.abs(gestureState.dy) > 5;
      }
      // When expanded, only capture downward drag at top of scroll
      if (scrollOffsetY.current <= 0 && gestureState.dy > 5) {
    ...
    onPanResponderMove: (_, gestureState) => {
      ...
      const collapsedTop = collapsedSheetTop;
      const range = collapsedTop - expandedTop;
      
      if (isSheetExpandedRef.current) {
        // Dragging down from expanded - only allow downward
        if (dragDistance > 0) {
          const progress = Math.max(0, Math.min(1, dragDistance / range));
          sheetAnim.setValue(1 - progress);
        }
      } else {
    ...
    onPanResponderRelease: (_, gestureState) => {
      ...
      // Calculate velocity for spring animation (native feel)
      const expandedTop = headerHeight;
      const collapsedTop = collapsedSheetTop;
      const range = collapsedTop - expandedTop;
      const velocityFactor = (velocity * 1000) / range;
      
      // Determine target state
      let shouldExpand: boolean;
      if (isSheetExpandedRef.current) {
        shouldExpand = !(dragDistance > 50 || velocity > 0.3);
      } else {
        shouldExpand = dragDistance < -50 || velocity < -0.3;
      }
      
      // Update ref immediately for gesture handling
      isSheetExpandedRef.current = shouldExpand;
      
      // Animate with velocity, update state when done
      Animated.spring(sheetAnim, {
        toValue: shouldExpand ? 1 : 0,
        velocity: Math.abs(velocityFactor),
        useNativeDriver: false,
        damping: 28,
        stiffness: 380,
        mass: 0.7,
  ...
  }), [headerHeight, collapsedSheetTop, sheetAnim, expandSheet, collapseSheet]);
  ...
  const handlePlacePress = useCallback((place: ListItem) => {
    if (place.type === 'Event') {
  ...
  const renderCard = useCallback(
    ...
    [handlePlacePress, nearMePermissionStatus, userCoords]
  );

  // Unified view - map always visible as background, sheet on top
  return (
    <View style={styles.container}>
      ...
      <MapView
        ...
        showsUserLocation={nearMePermissionStatus === ExpoLocation.PermissionStatus.GRANTED}
      >
        {mappablePlaces.map((place) => (
          <Marker
            ...
            pinColor={selectedMarkerPlace?.id === place.id ? Colors.light.tint : 'red'}
          />
        ))}
      </MapView>

      {/* Fixed header with solid background - always visible above sheet */}
      <View 
        style={[styles.fixedHeader, { paddingTop: insets.top + 12, backgroundColor: dynamicColors.background }]}
        onLayout={(e) => setHeaderHeight(e.nativeEvent.layout.height)}
      >
        <View style={styles.headerRow}>
          <TouchableOpacity style={[styles.headerIconButton, { backgroundColor: dynamicColors.cardBg, borderColor: dynamicColors.border }]} onPress={() => router.back()} activeOpacity={0.8}>
            <MingcuteIcon name="left_line" size={20} color={dynamicColors.text} />
          </TouchableOpacity>

          <View style={[styles.searchContainer, { backgroundColor: dynamicColors.cardBg, borderColor: dynamicColors.border }]}>
        ...
        {!isEmptySearchState && (
          <View style={styles.resultsHeader}>
            <Text style={[styles.resultsHeaderText, { color: dynamicColors.textSecondary }]}>
              <Text>Showing you </Text>
              <Text style={[styles.resultsHeaderBold, { color: dynamicColors.text }]}>{filteredPlaces.length} {filteredPlaces.length === 1 ? 'result' : 'results'}</Text>
              <Text> for </Text>
            ...
            <View style={styles.filtersRow}>
              ...
              <TouchableOpacity
                style={[styles.filterChip, { backgroundColor: dynamicColors.filterChipBg, borderColor: dynamicColors.border }, nearMe ? { backgroundColor: dynamicColors.filterChipActiveBg } : null]}
              ...
              <TouchableOpacity 
                style={[styles.filterChip, { backgroundColor: typeFilter !== 'All' ? dynamicColors.filterChipActiveBg : dynamicColors.filterChipBg, borderColor: dynamicColors.border }]} 
              ...
              {availableTags.length > 0 && (
                <TouchableOpacity 
                  style={[styles.filterChip, { backgroundColor: selectedTags.length > 0 ? dynamicColors.filterChipActiveBg : dynamicColors.filterChipBg, borderColor: dynamicColors.border }]} 
      ...
      {/* Location button - shows when sheet is collapsed and location available */}
      {userCoords && !isSheetExpanded && (
        <TouchableOpacity
      ...
      {/* Map button - fades with sheet animation */}
      <Animated.View 
        style={[styles.mapViewButton, { opacity: sheetAnim }]}
        pointerEvents={isSheetExpanded ? 'auto' : 'none'}
      >
        <TouchableOpacity 
          style={styles.mapViewButtonInner} 
          activeOpacity={0.9} 
      ...
      {/* Results sheet - custom animated view positioned below header */}
      <Animated.View 
        style={[styles.resultsSheet, { top: sheetTop, backgroundColor: dynamicColors.sheetBg }]}
        {...panResponder.panHandlers}
      >
        {/* Drag handle indicator */}
        <View style={styles.sheetDragHandle}>
          <View style={[styles.sheetDragIndicator, { backgroundColor: dynamicColors.dragIndicator }]} />
        </View>
        ...
        >
          {isEmptySearchState ? (
            <View style={styles.sheetEmptyState}>
              ...
              <View style={styles.quickSearchContainer}>
                ...
                <View style={styles.quickSearchButtons}>
                  <TouchableOpacity 
                    style={[styles.quickSearchButton, { backgroundColor: dynamicColors.filterChipBg, borderColor: dynamicColors.border }]} 
            ...
            <View style={{ gap: 10 }}>
              <ResultSkeletonCard isDark={isDark} />
              <ResultSkeletonCard isDark={isDark} />
              <ResultSkeletonCard isDark={isDark} />
            </View>
          ) : showNearMeDenied ? (
            <View style={styles.sheetEmptyState}>
              <Text style={[styles.listStateTitle, { color: dynamicColors.text }]}>Turn on location to use Near me.</Text>
              <TouchableOpacity
                activeOpacity={0.85}
            ...
            <View style={styles.sheetEmptyState}>
              <Text style={[styles.listStateTitle, { color: dynamicColors.text }]}>No results found.</Text>
      ...
      >
        <GorhomBottomSheetScrollView contentContainerStyle={styles.eventDetailsContent}>
          {selectedEvent && (
            <EventDetailsContent
      ...
      {/* Type Selection Native Sheet with Wheel Picker */}
      <NativeSheet ref={typeSheetRef} mediumOnly>
        <View style={styles.nativeSheetContent}>
          <View style={styles.nativeSheetHeader}>
            <Text style={[styles.nativeSheetTitle, { color: dynamicColors.text }]}>Select Type</Text>
            <TouchableOpacity 
              style={styles.nativeSheetDoneButton} 
              onPress={() => typeSheetRef.current?.dismiss()} 
              activeOpacity={0.8}
            >
              <Text style={styles.nativeSheetDoneButtonText}>Done</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              ...
              itemStyle={[styles.wheelPickerItem, { color: dynamicColors.text }]}
            >
      ...
      {/* Tags Selection Native Sheet */}
      <NativeSheet ref={tagsSheetRef} mediumOnly>
        <View style={styles.nativeSheetContent}>
          <View style={styles.nativeSheetHeader}>
            <Text style={[styles.nativeSheetTitle, { color: dynamicColors.text }]}>
              {typeFilter === 'All' ? 'Tags' : `${typeFilter} Tags`}
            </Text>
            <TouchableOpacity 
              style={styles.nativeSheetDoneButton} 
              onPress={() => tagsSheetRef.current?.dismiss()} 
              activeOpacity={0.8}
            >
              <Text style={styles.nativeSheetDoneButtonText}>Done</Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  nativeSheetDoneButton: {
    backgroundColor: 'rgba(120, 120, 128, 0.24)',
    paddingHorizontal: 20,
    paddingVertical: 10,
    borderRadius: 18,
  },
  nativeSheetDoneButtonText: {
    fontSize: 15,
    color: '#fff',
    fontFamily: 'Quicksand_600SemiBold',
  },
